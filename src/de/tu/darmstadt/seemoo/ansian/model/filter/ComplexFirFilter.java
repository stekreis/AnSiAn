...
/**
 * <h1>AnSiAn - complex FIR Filter</h1>
 *
 * Module: ComplexFirFilter.java Description: This class implements a FIR filter
 * with complex taps. Most of the code is copied from the firdes and firfilter
 * module from GNU Radio.
 *
 * @author Dennis Mantz
 *
...
public class ComplexFirFilter {
	...
	/**
	 * Private Constructor. Creates a new complex FIR Filter with the given taps
	 * and decimation. Use create*Filter() to calculate taps and create the
	 * filter.
	 * 
	 * @param tapsReal
	 *            filter taps real part
	 * @param tapsImag
	 *            filter taps imaginary part
	 * @param decimation
	 *            decimation factor
	 * @param gain
	 ...
	 * @param sampleRate
	 *            sample rate
	 * @param lowCutOffFrequency
	 *            lower cut off frequency (start of pass band)
	 * @param highCutOffFrequency
	 *            upper cut off frequency (end of pass band)
	 * @param transitionWidth
	 *            width from end of pass band to start stop band
	 * @param attenuation
	 *            attenuation of stop band
	 */
	private ComplexFirFilter(float[] tapsReal, float[] tapsImag, int decimation, float gain, float sampleRate,
			float lowCutOffFrequency, float highCutOffFrequency, float transitionWidth, float attenuation) {
	...
	public int filter(SamplePacket in, SamplePacket out, int offset, int length) {
		...
		for (int i = 0; i < length; i++) {
			...
			// Calculate the filter output for every Mth element (were M =
			// decimation)
			if (decimationCounter == 0) {
				// first check if we have enough space in the output buffers:
				if (indexOut == outputCapacity) {
					out.setSize(indexOut); // update size of output sample
											// packet
					out.setSampleRate(in.getSampleRate() / decimation); // update
																		// the
																		// sample
																		// rate
																		// of
																		...
																		// sample
																		// packet
					return i; // We return the number of consumed samples from
								// the input buffers
				}
		...
		out.setSampleRate(in.getSampleRate() / decimation); // update the sample
															// rate of the
															// output sample
															// packet
	 ...
	 *
	 * Will calculate the tabs for the specified low pass filter and return a
	 * FirFilter instance
	 *
	 * @param decimation
	 *            decimation factor
	 ...
	 *            cut off frequency (beginning of pass band)
	 ...
	 */
	public static ComplexFirFilter createBandPass(int decimation, float gain, float sampling_freq, // Hz
			...
			float attenuation_dB) // attenuation dB
	{
		...
		for (int i = 0; i < ntaps; i++) {
			...
			phase += freq;
			// Log.d(LOGTAG, "createBandPass: Filter Taps [i="+i+"]: " +
			// tapsReal[i] + " " + tapsImag[i]);
		}

		return new ComplexFirFilter(tapsReal, tapsImag, decimation, gain, sampling_freq, low_cutoff_freq,
				high_cutoff_freq, transition_width, attenuation_dB);
	}
