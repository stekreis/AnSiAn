...
/**
 * <h1>AnSiAn - FIR Filter</h1>
 *
 * Module: FirFilter.java Description: This class implements a FIR filter. Most
 * of the code is copied from the firdes and firfilter module from GNU Radio.
 *
 * @author Dennis Mantz
 *
...
public class FirFilter {
	...
	/**
	 * Private Constructor. Creates a new FIR Filter with the given taps and
	 * decimation. Use create*Filter() to calculate taps and create the filter.
	 * 
	 * @param taps
	 *            filter taps (double)
	 * @param decimation
	 ...
	 */
	private FirFilter(float[] taps, int decimation, float gain, float sampleRate, float cutOffFrequency,
			float transitionWidth, float attenuation) {
		this.taps = taps;
	...
	/**
	 * Filters the real parts of the samples from the input sample packet and
	 * appends filter output to the output sample packet. Stops automatically if
	 * output sample packet is full.
	 * 
	 * @param in
	 *            input sample packet
	 * @param out
	...
	public int filterReal(SamplePacket in, SamplePacket out, int offset, int length) {
		...
		int indexOut = out.size();
		int outputCapacity = out.capacity();
		float[] reIn = in.getRe(), reOut = out.getRe();
	 ...
	 */
	public static FirFilter createLowPass(int decimation, float gain, float sampling_freq, // Hz
			...
			float attenuation_dB) // attenuation dB
	{
		...
		// Log.e(LOGTAG,
		// "createLowPass: firdes check failed: sampling_freq > 0");
		// return null;
		// }
		//
		// if (cutoff_freq <= 0.0 || cutoff_freq > sampling_freq / 2) {
		// Log.e(LOGTAG,
		// "createLowPass: firdes check failed: 0 < fa <= sampling_freq / 2");
		// return null;
		// }
		//
		// if (transition_width <= 0) {
		// Log.e(LOGTAG,
		// "createLowPass: firdes check failed: transition_width > 0");
		// return null;
		// }
