...
/**
 * <h1>AnSiAn - unsigned 8-bit IQ Converter</h1>
 *
 * Module: Unsigned8BitIQConverter.java Description: This class implements
 * methods to convert the raw input data of IQ sources (8 bit unsigned) to
 * SamplePackets. It has also methods to do converting and down-mixing at the
 * same time.
 *
 * @author Dennis Mantz
 *
...
public class Unsigned8BitIQConverter extends IQConverter {
	...
	protected void generateLookupTable() {
		/**
		 * The rtl_sdr delivers samples in the following format: The bytes are
		 * interleaved, 8-bit, unsigned IQ samples (in-phase component first,
		 * followed by the quadrature component):
		 *
		 * [--------- first sample ----------] [-------- second sample --------]
		 * I Q I Q ... receivedBytes[0] receivedBytes[1] receivedBytes[2] ...
		 */
	...
	public int mixPacketIntoSamplePacket(byte[] packet, SamplePacket samplePacket, long channelFrequency) {
		...
		float[] im = samplePacket.getIm();
		for (int i = 0; i < packet.length; i += 2) {
			re[startIndex + count] = cosineRealLookupTable[cosineIndex][packet[i] & 0xff]
					- cosineImagLookupTable[cosineIndex][packet[i + 1] & 0xff];
			im[startIndex + count] = cosineRealLookupTable[cosineIndex][packet[i + 1] & 0xff]
					+ cosineImagLookupTable[cosineIndex][packet[i] & 0xff];
